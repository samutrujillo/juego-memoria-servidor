...
const BLOCK_ACTIVITY_LOG_FILE = path.join(__dirname, 'block-activity.log'); // Nuevo archivo de log
...
// Sistema de logging mejorado para seguimiento de bloqueos
function logBlockingActivity(userId, action, reason, source) {
    const user = getUserById(userId);
    if (!user) return;
    
    const timestamp = new Date().toISOString();
    const logEntry = `${timestamp} | ${user.username} (${userId}) | ${action} | Motivo: ${reason} | Origen: ${source} | Puntaje: ${user.score}\n`;
    ...
    try {
        fs.appendFileSync(
            BLOCK_ACTIVITY_LOG_FILE,
            logEntry
        );
    } catch (error) {
        console.error('Error al guardar log de bloqueo:', error);
    }
    
    // Opcionalmente guardar en Firebase
    if (db) {
        try {
            const logRef = db.ref('logs/blockActivity').push();
            logRef.set({
                timestamp: timestamp,
                userId: userId,
    ...
    for (const username of usersToCheck) {
        ...
        if (user) {
            ...
            if (user.isBlocked || user.isLockedDueToScore !== shouldBeBlocked) {
                ...
                logBlockingActivity(
                    user.id,
                    shouldBeBlocked ? 'BLOQUEO_AUTOMÁTICO_CORREGIDO' : 'DESBLOQUEO_AUTOMÁTICO_CORREGIDO',
                    `Corrección específica para usuario problemático`,
                    'unblockSpecificUsers'
                );
                
                // Actualizar en Firebase
                if (db) {
                    queueGameStateChange(`gameState/userScores/${user.id}/isBlocked`, false);
                ...
                if (playerInfo && playerInfo.socketId) {
                    io.to(playerInfo.socketId).emit('blockStatusChanged', {
                        isBlocked: false,
                        isLockedDueToScore: shouldBeBlocked,
                        message: shouldBeBlocked 
                            ? 'Tu cuenta está bloqueada por puntaje (menos de 23,000 puntos).' 
...
// Verificar y corregir estados de bloqueo incorrectos
function verifyAndCorrectBlockStatus() {
    console.log("Verificando estados de bloqueo de todos los usuarios...");
    let correctedCount = 0;
    
    for (const user of users) {
        if (user.isAdmin) continue; // Ignorar administradores
        
        // Verificar si el bloqueo es correcto según las reglas
        const shouldBeLockedDueToScore = user.score <= 23000;
        
        // ESPECIAL: Verificación forzada para Orion y Nova para asegurar desbloqueo correcto
        if (user.username === "Orion" || user.username === "Nova") {
            // Si tienen más de 23000 puntos, asegurar que no estén bloqueados por ningún motivo
            if (user.score > 23000 && (user.isBlocked || user.isLockedDueToScore)) {
                console.log(`Corrigiendo estado de bloqueo para ${user.username} (usuario especial)`);
                user.isBlocked = false;
                ...
                logBlockingActivity(
                    user.id, 
                    'DESBLOQUEO_USUARIO_ESPECIAL', 
                    `Corrección de usuario especial con puntaje: ${user.score}`, 
                    'verifyAndCorrectBlockStatus'
                );
            ...
            continue; // Continuar con el siguiente usuario después de verificar especialmente estos
        }
        
        // Corregir estado si es incorrecto para usuarios regulares
        if (user.isLockedDueToScore !== shouldBeLockedDueToScore) {
            console.log(`Corrigiendo estado de bloqueo por puntaje para ${user.username}: ${user.isLockedDueToScore} → ${shouldBeLockedDueToScore}`);
            user.isLockedDueToScore = shouldBeLockedDueToScore;
            correctedCount++;
            
            logBlockingActivity(
                user.id, 
                shouldBeLockedDueToScore ? 'BLOQUEO_CORREGIDO' : 'DESBLOQUEO_CORREGIDO', 
                `Corrección automática, puntaje: ${user.score}`, 
                'verifyAndCorrectBlockStatus'
            );
            ...
            if (playerSocketId) {
                if (shouldBeLockedDueToScore) {
                    ...
                    io.to(playerSocketId).emit('userUnlocked', {
                        message: 'Tu cuenta ha sido desbloqueada debido a que tu puntaje superó los 23,000 puntos.'
                    });
                }
                ...
                io.to(playerSocketId).emit('blockStatusChanged', {
                    ...
                    message: shouldBeLockedDueToScore ? 
                        'Tu cuenta ha sido bloqueada por alcanzar 23,000 puntos o menos.' : 
                        'Tu cuenta ha sido desbloqueada debido a que tu puntaje superó los 23,000 puntos.'
                });
            }
        }
    }
    ...
    if (correctedCount > 0) {
        console.log(`Se corrigieron ${correctedCount} estados de bloqueo incorrectos`);
        saveGameState();
    } else {
        console.log("No se encontraron estados de bloqueo incorrectos");
    }
}

// Ejecutar esta validación periódicamente
setInterval(validateBoardIntegrity, 5 * 60 * 1000); // Cada 5 minutos
setInterval(verifyAndCorrectBlockStatus, 2 * 60 * 1000); // Cada 2 minutos
setInterval(unblockSpecificUsers, 30 * 1000); // Cada 30 segundos
...
function checkScoreLimit(user) {
    // ESPECIAL: Para Orion y Nova, verificación adicional
    if (user.username === "Orion" || user.username === "Nova") {
        // Asegurar que solo estén bloqueados por puntaje, nunca por otras razones
        user.isBlocked = false;
        user.isLockedDueToScore = user.score <= 23000;
        
        if (user.isLockedDueToScore) {
            console.log(`Usuario especial ${user.username} bloqueado SOLAMENTE por puntaje (${user.score} ≤ 23000)`);
        } else {
            console.log(`Usuario especial ${user.username} NO bloqueado (puntaje ${user.score} > 23000)`);
        }
        
        // Actualizar Firebase
        if (db) {
            queueGameStateChange(`gameState/userScores/${user.id}/isBlocked`, false);
            queueGameStateChange(`gameState/userScores/${user.id}/isLockedDueToScore`, user.isLockedDueToScore);
        }
        ...
        if (playerSocketId) {
            ...
            io.to(playerSocketId).emit('blockStatusChanged', {
                ...
                message: user.isLockedDueToScore ? 
                    'Tu cuenta ha sido bloqueada por puntaje bajo.' : 
                    'Tu cuenta está activa.'
            });
        }
        
        return user.isLockedDueToScore;
    }
    
    // Para usuarios normales
    if (user.score <= 23000 && !user.isAdmin) {
        const wasLockedBefore = user.isLockedDueToScore;
        ...
        if (!wasLockedBefore) {
            logBlockingActivity(
                user.id, 
                'BLOQUEO_AUTOMATICO', 
                `Puntaje (${user.score}) menor o igual a 23,000`, 
                'checkScoreLimit'
            );
        }
        ...
        if (playerSocketId) {
            ...
            io.to(playerSocketId).emit('blockStatusChanged', {
                ...
                // IMPORTANTE: No tocar otros estados de bloqueo
                message: 'Has alcanzado o caído a 23,000 puntos o menos. Tu cuenta ha sido bloqueada temporalmente.'
            });
        }
        ...
        if (db) {
            ...
            // NO actualizar otros estados de bloqueo
        }
    ...
    // AGREGADO: Si tenía bloqueo por puntuación y ahora tiene más de 23000, desbloquear automáticamente
    if (user.score > 23000 && user.isLockedDueToScore && !user.isAdmin) {
        console.log(`Usuario ${user.username} desbloqueado automáticamente por superar 23,000 puntos (actual: ${user.score})`);
        user.isLockedDueToScore = false;
        
        logBlockingActivity(
            user.id, 
            'DESBLOQUEO_AUTOMATICO', 
            `Puntaje (${user.score}) superó 23,000`, 
            'checkScoreLimit'
        );
        ...
        if (playerSocketId) {
            io.to(playerSocketId).emit('userUnlocked', {
                message: 'Tu cuenta ha sido desbloqueada automáticamente porque tu puntaje superó 23,000 puntos.'
            });
            io.to(playerSocketId).emit('blockStatusChanged', {
...
async function saveGameState() {
    // NUEVO: Verificar y corregir datos antes de guardar
    const sanitizedPlayers = gameState.players.map(player => ({
        id: player.id || 'unknown',
        username: player.username || 'unknown',
        socketId: player.socketId || null, // Garantizar que socketId nunca sea undefined
        isConnected: player.isConnected || false
    }));
    
    const stateToSave = {
        board: gameState.board,
        players: sanitizedPlayers, // Usar los datos sanitizados
    ...
    try {
     ...
     async function loadGameState() {
        ...
        if (loadedState) {
            ...
            if (loadedState.userScores) {
                for (const userId in loadedState.userScores) {
                    ...
                    if (user) {
                        ...
                        // ESPECIAL: Corrección para Orion y Nova
                        if (user.username === "Orion" || user.username === "Nova") {
                            // Solo bloquear por puntaje si corresponde
                            if (user.score > 23000) {
                                user.isBlocked = false;
                                user.isLockedDueToScore = false;
                            }
            ...
            if (loadedState.players) {
                // Asegurar que los objetos de player tengan todos los campos necesarios
            ...
            // 6. Verificar y corregir estados de bloqueo
            verifyAndCorrectBlockStatus();
            
            // 7. Verificar específicamente Orion y Nova
            unblockSpecificUsers();
        ...
        // Desbloquear usuarios Orion y Nova
        users.forEach(user => {
            if (user.username === "Orion" || user.username === "Nova") {
                user.isBlocked = false;
                user.isLockedDueToScore = false;
     ...
     (async function () {
        try {
            ...
            // Forzar verificación de usuarios Orion y Nova
            unblockSpecificUsers();
        } catch (err) {
     ...
     async function resetBoardOnly() {
        ...
        // MODIFICADO: Verificar qué jugadores están realmente conectados sin modificar estados de bloqueo
        ...
        // MODIFICADO: Actualizar SOLO el estado de conexión, no el estado de bloqueo
        for (const player of gameState.players) {
            // Mantener el estado de conexión actualizado sin afectar el estado de bloqueo
            player.isConnected = connectedPlayerIds.has(player.id);
            
            // AGREGADO: Asegurar que no haya bloqueos automáticos
            const userObject = getUserById(player.id);
            if (userObject && !userObject.isAdmin) {
                // ESPECIAL: Para Orion y Nova, verificar y corregir
                if (userObject.username === "Orion" || userObject.username === "Nova") {
                    if (userObject.score > 23000) {
                        // Si tienen suficientes puntos, asegurar desbloqueo
                        userObject.isBlocked = false;
                        userObject.isLockedDueToScore = false;
                        console.log(`Asegurando desbloqueo de usuario especial ${userObject.username} al cambiar de mesa`);
                    }
                } else {
                    // Para otros usuarios, SOLO actualizar isLockedDueToScore si debe bloquearse por puntaje
                    if (userObject.score <= 23000) {
                        userObject.isLockedDueToScore = true;
                    }
                }
                // NUNCA actualizar isBlocked automáticamente
            }
        }
     
        // MODIFICADO: Si solo hay un jugador conectado, establecerlo como el jugador actual
            ...
            try {
                const criticalUpdates = {
                    ...
                    'gameState/players': gameState.players.map(p => ({
                        id: p.id,
                        username: p.username,
                        socketId: p.socketId || null, // Asegurar que nunca sea undefined
                        isConnected: p.isConnected
                        // IMPORTANTE: No enviar ni actualizar estado de bloqueo aquí
                    })),
        ...
        // MODIFICADO: Asegurar que solo se envía la lista de jugadores conectados sin estados de bloqueo
        io.emit('boardReset', {
        ...
        for (const player of gameState.players) {
            ...
            // Incrementar contador solo para jugadores conectados que no son admin
            if (player.isConnected && !getUserById(playerId)?.isAdmin) {
            ...
            if (playerSocketId && player.isConnected) {
                ...
                // Si alcanzó el límite, notificar pero NO bloquear
        ...
        // MODIFICADO: Asegurar que se envía el estado correcto de bloqueo
        io.emit('gameState', {
            board: gameState.board,
            ...
            players: gameState.players.map(player => {
                ...
                return {
                    ...
                    // Usar SOLO el estado de bloqueo de los datos de usuario, no modificar aquí
                    isBlocked: userData?.isBlocked || false,
                    isLockedDueToScore: userData?.isLockedDueToScore || false,
                    isConnected: player.isConnected
                };
            }),
            status: 'playing',
        ...
        // Verificar estados de bloqueo después del cambio de mesa
        verifyAndCorrectBlockStatus();
        unblockSpecificUsers();
     ...
     async function resetGame() {
        ...
        // Verificar y corregir estados específicos
        unblockSpecificUsers();
     ...
     // Función para sincronizar el estado del jugador - mejorada para evitar bloqueos automáticos
     function syncPlayerState(userId, socketId) {
        const user = getUserById(userId);
        if (!user) return;
     
        // ESPECIAL: Para Orion y Nova, asegurar desbloqueo si tienen suficientes puntos
        if (user.username === "Orion" || user.username === "Nova") {
            if (user.score > 23000) {
                user.isBlocked = false;
                user.isLockedDueToScore = false;
                console.log(`Desbloqueando usuario especial ${user.username} durante sincronización`);
            }
        }
     
        // IMPORTANTE: Solo sincronizar el estado real del usuario sin modificarlo
        // excepto por la regla de puntaje
     
        // Verificar y posiblemente actualizar bloqueo por puntaje
        checkScoreLimit(user);
...
io.to(socketId).emit('gameState', {
    ...
    players: gameState.players.map(player => {
        ...
        return {
            id: player.id,
            username: player.username,
            // Usar SOLO el estado real del usuario sin modificarlo
            isBlocked: userData?.isBlocked || false,
            isLockedDueToScore: userData?.isLockedDueToScore || false,
            isConnected: player.isConnected
        };
...
// NUEVO: Validar y limpiar estados de jugadores antes de procesar
for (const player of gameState.players) {
    // Asegurarse de que todos los jugadores tengan socketId válido
    if (!player.socketId) {
        // Buscar si existe una conexión activa para este jugador
        for (const socketId in connectedSockets) {
            if (connectedSockets[socketId] === player.id) {
                console.log(`Actualizando socketId para ${player.username}: ${socketId}`);
    ...
    // CRÍTICO: Verificar explícitamente los usuarios Orion y Nova para desbloquear
    const userData = getUserById(player.id);
    if (userData && (userData.username === "Orion" || userData.username === "Nova")) {
        // Solo bloquear si el puntaje es <= 23000
        userData.isLockedDueToScore = userData.score <= 23000;
        
        // Desbloqueo manual para estos usuarios específicos si no están bloqueados por puntaje
        if (userData.score > 23000) {
            console.log(`Desbloqueando usuario específico: ${userData.username}`);
            userData.isBlocked = false;
            userData.isLockedDueToScore = false;
            
            // Notificar al cliente sobre el desbloqueo
            if (player.socketId) {
                io.to(player.socketId).emit('blockStatusChanged', {
                    isBlocked: false,
                    isLockedDueToScore: false,
...
// MODIFICADO: Filtrar jugadores conectados para turnos sin modificar estados
let eligiblePlayers = gameState.players.filter(player => {
    const userData = getUserById(player.id);
    // Solo verificar el estado actual sin modificarlo
    return player.isConnected && userData && 
           !userData.isBlocked && 
...
if (eligiblePlayers.length === 1) {
    ...
    while (!nextPlayerFound && loopCount < gameState.players.length) {
        ...
        // Sin modificar su estado de bloqueo
    ...
    // Si no encontramos un jugador válido, mantener el índice original sin bloquear a nadie
    if (!nextPlayerFound) {
...
// MODIFICADO: Asegurar que se envían los estados correctos de bloqueo
io.emit('gameState', {
    board: gameState.board.map(tile => ({
        ...tile,
        value: tile.revealed ? tile.value : null
    ...
    players: gameState.players.map(player => {
        ...
        return {
            id: player.id,
            username: player.username,
            // Usar el estado de bloqueo de los datos de usuario, no modificar aquí
            isBlocked: userData?.isBlocked || false,
            isLockedDueToScore: userData?.isLockedDueToScore || false,
            isConnected: player.isConnected
        };
...
socket.on('reconnectUser', ({ userId, username }) => {
    ...
    if (playerIndex !== -1) {
        ...
        // ESPECIAL: Verificar usuarios críticos
        const user = getUserById(userId);
        if (user && (user.username === "Orion" || user.username === "Nova")) {
            if (user.score > 23000) {
                user.isBlocked = false;
                user.isLockedDueToScore = false;
                console.log(`Desbloqueando usuario especial ${user.username} en reconexión`);
                
                io.to(socket.id).emit('blockStatusChanged', {
                    isBlocked: false,
                    isLockedDueToScore: false,
                    message: 'Tu cuenta ha sido desbloqueada.'
                });
...
socket.on('syncGameState', ({ userId }) => {
    ...
    // ESPECIAL: Para Orion y Nova, asegurar desbloqueo si tienen puntos
    if (user.username === "Orion" || user.username === "Nova") {
        if (user.score > 23000) {
            user.isBlocked = false;
            user.isLockedDueToScore = false;
            console.log(`Desbloqueando usuario especial ${user.username} en syncGameState`);
        }
    }
...
// Evento para sincronización forzada
socket.on('forceSyncRequest', ({ userId }) => {
    ...
    if (userId === connectedSockets[socket.id]) {
        // Obtener el usuario real
        const user = getUserById(userId);
        if (!user) return;
        
        // MODIFICADO: Solo limpiar estados locales sin modificar bloqueos
        // excepto por la regla de puntaje
        ...
        if (user.username === "Orion" || user.username === "Nova") {
            if (user.score > 23000) {
                user.isBlocked = false;
                user.isLockedDueToScore = false;
                console.log(`Desbloqueando usuario especial ${user.username} en forceSyncRequest`);
            ...
            // Para otros usuarios, verificar bloqueo por puntaje
            checkScoreLimit(user);
        }
        
        // Limpiar estados locales del juego sin afectar bloqueos
        const playerSelections = initPlayerSelections(userId);
        
        // Generar nuevo tablero local si es necesario
        if (!gameState.board || gameState.board.length < 16) {
            const newBoard = generateBoard();
            gameState.board = newBoard;
        }
        ...
        socket.emit('gameState', {
            ...
            players: gameState.players.map(player => {
                ...
                return {
                    id: player.id,
                    username: player.username,
                    // Usar SOLO el estado actual de bloqueo
                    isBlocked: userData?.isBlocked || false,
                    isLockedDueToScore: userData?.isLockedDueToScore || false,
                    isConnected: player.isConnected
                };
            ...
            status: 'playing',
            rowSelections: playerSelections.rowSelections || [0, 0, 0, 0]
        });
...
socket.on('login', (credentials, callback) => {
    ...
    // ESPECIAL: Para Orion y Nova, asegurar desbloqueo al iniciar sesión si tienen puntos
    if ((user.username === "Orion" || user.username === "Nova") && user.score > 23000) {
        user.isBlocked = false;
        user.isLockedDueToScore = false;
        console.log(`Desbloqueando usuario especial ${user.username} en inicio de sesión`);
    }
...
socket.on('joinGame', () => {
    ...
    // ESPECIAL: Para Orion y Nova, asegurar desbloqueo al unirse al juego si tienen puntos
    if ((user.username === "Orion" || user.username === "Nova") && user.score > 23000) {
        user.isBlocked = false;
        user.isLockedDueToScore = false;
        console.log(`Desbloqueando usuario especial ${user.username} al unirse al juego`);
    }
...
socket.on('selectTile', async ({ tileIndex, currentScore }) => {
    ...
    // ESPECIAL: Para Orion y Nova, asegurar desbloqueo antes de verificar
    if ((user.username === "Orion" || user.username === "Nova") && user.score > 23000) {
        user.isBlocked = false;
        user.isLockedDueToScore = false;
    }
    ...
    if (checkTableLimit(userId)) {
       ...
       if (checkGameOver()) {
           ...
           // Verificar usuarios específicos después del cambio de mesa
           unblockSpecificUsers();
   ...
   socket.on('syncScore', async ({ userId }) => {
       ...
       const user = getUserById(userId);
       if (user) {
           // ESPECIAL: Para Orion y Nova, verificar desbloqueo
           if ((user.username === "Orion" || user.username === "Nova") && user.score > 23000) {
               user.isBlocked = false;
               user.isLockedDueToScore = false;
               console.log(`Desbloqueando usuario especial ${user.username} en sincronización de puntaje`);
   ...
   socket.on('rechargePoints', async ({ userId }, callback) => {
       ...
       // NUEVO: Si es Orion o Nova, asegurar desbloqueo completo
       if (targetUser.username === "Orion" || targetUser.username === "Nova") {
           targetUser.isLockedDueToScore = false;
           console.log(`Administrador desbloqueó usuario especial ${targetUser.username}`);
       }
       
       // Registrar actividad de desbloqueo
       logBlockingActivity(
           userId,
           'DESBLOQUEO_ADMIN',
           'Recarga de puntos por admin',
           'rechargePoints'
       );
       ...
       if (db) {
           try {
               await db.ref(`gameState/userScores/${userId}`).update({
                   ...
                   isBlocked: false,
                   isLockedDueToScore: false // Asegurar desbloqueo completo
               });
   ...
   socket.on('updatePoints', async ({ userId, points }, callback) => {
       ...
       // ESPECIAL: Para Orion y Nova, verificar desbloqueo automático
       if ((targetUser.username === "Orion" || targetUser.username === "Nova") && targetUser.score > 23000) {
           targetUser.isBlocked = false;
           targetUser.isLockedDueToScore = false;
           console.log(`Desbloqueando usuario especial ${targetUser.username} después de actualizar puntos`);
       }
       ...
       if (playerSocketId) {
           ...
           // Notificar cambio de estado si es Orion o Nova
           if (targetUser.username === "Orion" || targetUser.username === "Nova") {
               io.to(playerSocketId).emit('blockStatusChanged', {
                   isBlocked: targetUser.isBlocked,
                   isLockedDueToScore: targetUser.isLockedDueToScore,
                   message: 'El administrador ha actualizado tu puntaje.'
               });
           }
       }
   ...
   socket.on('toggleBlockUser', async ({ userId }, callback) => {
       ...
       const previousState = targetUser.isBlocked;
       targetUser.isBlocked = !targetUser.isBlocked;
       
       // Registrar la actividad de bloqueo/desbloqueo
       logBlockingActivity(
           userId, 
           targetUser.isBlocked ? 'BLOQUEO_MANUAL' : 'DESBLOQUEO_MANUAL', 
           `Acción manual de administrador (${admin.username})`, 
           'toggleBlockUser'
       );
   ...
   socket.on('unlockUserScore', async ({ userId }, callback) => {
       ...
       // Desbloquear al usuario
       const previousState = targetUser.isLockedDueToScore;
       targetUser.isLockedDueToScore = false;
       
       // ESPECIAL: Asegurar desbloqueo total para Orion y Nova
       if (targetUser.username === "Orion" || targetUser.username === "Nova") {
           targetUser.isBlocked = false;
           console.log(`Administrador desbloqueó usuario especial ${targetUser.username} por puntaje`);
       }
       
       // Registrar la actividad de desbloqueo
       logBlockingActivity(
           userId,
           'DESBLOQUEO_ADMIN_PUNTAJE',
           `Desbloqueo manual por admin (${admin.username})`,
           'unlockUserScore'
       );
       ...
       if (db) {
           try {
               const updates = {
                   [`gameState/userScores/${userId}/isLockedDueToScore`]: false
               };
               
               // Añadir desbloqueo regular si es Orion o Nova
               if (targetUser.username === "Orion" || targetUser.username === "Nova") {
                   updates[`gameState/userScores/${userId}/isBlocked`] = false;
               }
   ...
   socket.on('resetGame', async (callback) => {
       ...
       // Verificar y desbloquear usuarios específicos después del reinicio
       unblockSpecificUsers();
   ...
   socket.on('directSetPoints', async ({ userId, newPoints }, callback) => {
       ...
       logBlockingActivity(
           userId,
           'AJUSTE_PUNTOS_DIRECTO',
           `Establecimiento directo de puntos por admin (${admin.username}): ${newPoints}`,
           'directSetPoints'
       );
       ...
       if ((targetUser.username === "Orion" || targetUser.username === "Nova") && targetUser.score > 23000) {
           targetUser.isBlocked = false;
           targetUser.isLockedDueToScore = false;
           console.log(`Desbloqueando usuario especial ${targetUser.username} después de ajuste directo de puntos`);
       }
   ...
   socket.on('unlockTables', async ({ userId }, callback) => {
       ...
       logBlockingActivity(
           userId,
           'DESBLOQUEO_MESAS',
           `Desbloqueo de mesas por admin (${admin.username})`,
           'unlockTables'
       );
   ...
   // Comando especial para desbloquear usuarios específicos (Orion y Nova)
   socket.on('forceUnblockSpecialUsers', async (callback) => {
    const adminId = connectedSockets[socket.id];
    if (!adminId) {
        callback({ success: false, message: 'No autorizado' });
        return;
    ...
    // Buscar y desbloquear usuarios especiales
    let unblocked = 0;
    for (const user of users) {
        if (user.username === "Orion" || user.username === "Nova") {
            ...
            if (wasBlocked) {
                ...
                logBlockingActivity(
                    user.id,
                    'DESBLOQUEO_FORZADO',
                    `Comando especial de admin para usuario problemático`,
                    'forceUnblockSpecialUsers'
                );
                ...
                if (db) {
                    try {
                        const updates = {
                            [`gameState/userScores/${user.id}/isBlocked`]: false,
                            [`gameState/userScores/${user.id}/isLockedDueToScore`]: false
                        };
                        await db.ref().update(updates);
                        console.log(`Usuario especial ${user.username} desbloqueado en Firebase`);
                    } catch (error) {
                        console.error(`Error al desbloquear usuario especial en Firebase:`, error);
                    }
                }
            }
        }
    ...
    callback({ 
        success: true, 
        message: `Se desbloquearon ${unblocked} usuarios especiales` 
    });
});
...
// Verificación periódica de usuarios específicos
setInterval(() => {
// Verificar Orion y Nova
unblockSpecificUsers();
...
// Endpoint para forzar desbloqueo de usuarios específicos
app.get('/unblock-special-users', async (req, res) => {
try {
    // Buscar y desbloquear usuarios Orion y Nova
    let unblocked = 0;
    for (const user of users) {
        if (user.username === "Orion" || user.username === "Nova") {
            const wasBlocked = user.isBlocked || user.isLockedDueToScore;
            ...
            if (wasBlocked) {
                ...
                logBlockingActivity(
                    user.id,
                    'DESBLOQUEO_FORZADO_ENDPOINT',
                    `Desbloqueo forzado mediante endpoint`,
                    'unblock-special-users'
                );
    ...
    res.status(500).json({
        success: false,
        message: 'Error al desbloquear usuarios especiales',
        error: error.message
    });
}
});
...
// Inicializar la verificación de usuarios especiales al inicio
unblockSpecificUsers();
...
// Verificar estados de bloqueo al inicio
verifyAndCorrectBlockStatus();
});
