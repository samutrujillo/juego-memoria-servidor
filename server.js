...
// Función para verificar y aplicar usuarios modificados
function verifyModifiedUsers() {
    console.log("Verificando usuarios modificados...");
    
    let changesApplied = 0;
    
    users.forEach(user => {
        if (user.usernameModified || user.passwordModified) {
            console.log(`Usuario ${user.id} tiene modificaciones persistentes: username=${user.usernameModified}, password=${user.passwordModified}`);
            changesApplied++;
        }
    });
    
    if (changesApplied > 0) {
        console.log(`Se aplicaron ${changesApplied} modificaciones de usuario desde almacenamiento persistente`);
    }
...
async function saveGameState() {
    // Crear objeto para usuarios modificados (solo los que han cambiado)
    const modifiedUsers = {};
    users.forEach(user => {
        // Solo guardar usuarios que tienen cambios
        if (user.usernameModified || user.passwordModified) {
            modifiedUsers[user.id] = {
                username: user.username,
                password: user.password,
                usernameModified: user.usernameModified || false,
                passwordModified: user.passwordModified || false
            };
        }
    });
    ...
    const stateToSave = {
        ...
        modifiedUsers: modifiedUsers, // NUEVO: Guardar usuarios modificados
...
async function loadGameState() {
    ...
    if (loadedState) {
       ...
       // NUEVO: Cargar usuarios modificados
       if (loadedState.modifiedUsers) {
           for (const userId in loadedState.modifiedUsers) {
               const user = users.find(u => u.id === userId);
               if (user) {
                   // Aplicar cambios persistentes de username y password
                   if (loadedState.modifiedUsers[userId].username) {
                       user.username = loadedState.modifiedUsers[userId].username;
                       user.usernameModified = loadedState.modifiedUsers[userId].usernameModified || true;
                       console.log(`Cargando username modificado para ${userId}: ${user.username}`);
                   }
                   if (loadedState.modifiedUsers[userId].password) {
                       user.password = loadedState.modifiedUsers[userId].password;
                       user.passwordModified = loadedState.modifiedUsers[userId].passwordModified || true;
                       console.log(`Cargando password modificado para ${userId}`);
                   }
               }
           }
       }
...
(async function () {
   try {
       ...
       // NUEVO: Verificar usuarios modificados después de cargar
       verifyModifiedUsers();
   } catch (err) {
...
socket.on('unlockAllTables', async (_, callback) => {
   ...
   // Evento para cambiar el nombre de usuario (solo para admins) - MODIFICADO
   socket.on('changeUsername', async ({ userId, newUsername }, callback) => {
       ...
       targetUser.usernameModified = true; // NUEVO: Marcar como modificado
       ...
       if (db) {
           try {
               await db.ref(`gameState/modifiedUsers/${userId}`).update({
   ...
   // Evento para cambiar la contraseña (solo para admins) - MODIFICADO
   socket.on('changePassword', async ({ userId, newPassword }, callback) => {
       ...
       targetUser.passwordModified = true; // NUEVO: Marcar como modificado
